...
function chessboard = growChessboard2(chessboard,corners,border_type)
debug = 2;
         ...
         if debug > 1
          ...
          if  chessboardEnergy(chessboard,corners) < -10
               cb = chessboard;
                for k=1:size(cb,1)
                    p = corners.p(cb(k,:),:) + 1;
                    scatter(p(:,1),p(:,2),18,'m','filled','LineWidth',0.1);
                end
              scatter(pred(:,1),pred(:,2),'c','filled');
          end
%           debug
         end
      end
  ...
  case 2
     %ste: 棋盘后三行的点
      if size(chessboard,1)<4
        pred = predictCorners(p(chessboard(end-2,:),:),p(chessboard(end-1,:),:),p(chessboard(end,:),:));
      else
          pred = predictCorners2(chessboard,2,p);
      end
        idx = assignClosestCorners(cand,pred);
        if debug > 2
        %debug
              if  chessboardEnergy(chessboard,corners) < -10
                  ...
                  scatter(pred(:,1),pred(:,2),'g','filled');
              end
    %           debug
        end
        if idx~=0
  ...
  case 3
      ...
      if size(chessboard,2)<4
        pred = predictCorners(p(chessboard(:,3),:),p(chessboard(:,2),:),p(chessboard(:,1),:));
          ...
          pred = predictCorners2(chessboard,3,p);
        %debug
          if debug > 3
          
              if  chessboardEnergy(chessboard,corners) < -5
              cb = chessboard;
                    for k=1:size(cb,1)
                        p = corners.p(cb(k,:),:) + 1;
                        scatter(p(:,1),p(:,2),18,'m','filled','LineWidth',0.1);
                    end
                  scatter(pred(:,1),pred(:,2),'y','filled');
              end
          end
          %debug
      end
  ...
  case 4
      ...
      if size(chessboard,1)<4
          ...
          pred = predictCorners2(chessboard,4,p);
      end
         idx = assignClosestCorners(cand,pred);
        if debug > 4
        %debug
