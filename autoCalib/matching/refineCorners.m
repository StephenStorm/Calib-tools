...
for i=1:size(corners.p,1)
  ...
  cu = corners.p(i,1);      %col     explanation in onMaxiumSuppression
  cv = corners.p(i,2);      %row
  ...
  for u=max(cu-r,1):min(cu+r,width)
    for v=max(cv-r,1):min(cv+r,height)
      ...
      if u~=cu || v~=cv % do not consider center pixel
        ...
        d1 = norm(w-w*v1'*v1);
        d2 = norm(w-w*v2'*v2);
        %ste: w*v1'*v1  得到的是w在单位向量V1方向的投影向量
        %ste: 此处个人理解为当前点到投影v[1,2]方向的距离。
